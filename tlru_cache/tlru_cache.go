...
import (
	...
	"github.com/preston-wagner/go-cache/mem_usage"
...
func NewTLRUCache[KEY_TYPE comparable, VALUE_TYPE any](
	ttl time.Duration,
	reapFrequency time.Duration,
	memoryLimit float64,
) *TLRUCache[KEY_TYPE, VALUE_TYPE] {
...
func (tlruCache *TLRUCache[KEY_TYPE, VALUE_TYPE]) Get(key KEY_TYPE) (VALUE_TYPE, bool) {
	...
	if ok {
		...
		return defaults.ZeroValue[VALUE_TYPE](), ok
	}
}
...
func (tlruCache *TLRUCache[KEY_TYPE, VALUE_TYPE]) Reap() {
	if tlruCache.Len() > 0 {
		...
		if mem_usage.MemUsage() >= tlruCache.memoryLimit {
		...
		tlruCache.cache = maps.FilterMap(tlruCache.cache, filter)
	}
}
